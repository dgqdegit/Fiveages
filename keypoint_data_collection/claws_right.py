...
def main():
    ...
    try:
        while True:
            ...
            # 提示用户按下 'a' 键开始运动
            print("按下 'q' 键轨迹复现, 'i'键回到初始位置, 'e'键夹爪关闭, 'r'键夹爪打开...")
            ...
            if replay_motion:
                ...
                # initial_pose = f"{{140.3677,-293.0568,94.2646,179.1168,-0.8176,175.6116}}" # （适用于 方块放盘子任务 and 按压洗发水任务）
                # initial_pose = f"{{60.0710,-440.3937,153.1288,91.8859,-1.8677,-30.7417}}" # （适用于  瓶子放进微波炉任务）
                initial_pose = f"{{-248.7451,-627.7677,350.2239,-93.5476,-84.9201,-179.5557}}" # （适用于  抓取动物放进抽屉任务）
                send_movj_command(server.sock, initial_pose)
            elif claw_close:
...
def replay_motion_trajectory(sock, modbus, timestamp, replay = True):
    ...
    try:
        ...
        if len(trajectory_points) >=7:
            print(f"开始轨迹复现：{os.path.join('data/right_wbl/', folder_path, 'pose.txt')}")

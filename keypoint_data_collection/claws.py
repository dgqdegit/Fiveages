    ...
    def connect_to_server(self):
        ...
        while True:
            try:
                ...
                self.app.connect((self.host, self.app_port))
                return self.app
    ...
    def listen_for_data(self):
        ...
        try:
            ...
            while True:
                # 接收数据
                if not self.app:
                    self.app = self.connect_to_server()
                if self.app:
                    data = self.app.recv(1024).decode().strip()
                    print(data)
                self.app.getpeername()
                if data == 'replay':
                    replay_motion = True
                elif data == 'open':
                    claw_open = True
...
def main():
    ...
    global claw_close
    # 创建一个 TCP/IP 套接字
    server = Server('192.168.201.1', 29999, '192.168.110.23', 12345)
    ...
    listener.start()
    listen_app = threading.Thread(target = server.listen_for_data, args = ())
    listen_app.start()
    try:
        while True:
            # 定义 p2 到 p6 的固定点位
            ...
            # 等待用户按下 'a' 键
            while not (replay_motion or claw_open or claw_close):
                pass
            print("开始机械臂运动...")

...
class DobotController():
    ...
    def move_joint(self, pose: str, a=30, v=30):
        """关节运动指令封装"""
        self.send_command(f"MovJ(pose={pose},a={a},v={v})")
    def interrupt_close(self):
        """中断错误"""
        self.send_modbus_command('Modbus', f'close({self.modbus})')
        self.send_modbus_command('ModbusRTU', f'close({self.modbusRTU})')
    @property
    def status(self) -> int:
        """获取机械臂状态"""

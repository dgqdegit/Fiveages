    ...
    def calib(self, verbose: bool = True, save: bool = False, e2h: bool = True) -> np.ndarray:  
        ...
        while True:  
            ...
            if ret:  
                ...
                if key == ord("q"):  
                    ...
                    # 收集图像点和对象点
                    imgpoints.append(corners2)  # 保存检测到的角点
                    objpoints.append(objp)  # 保存对象点
        ...
        for i in range(len(objpoints)):
            imgpoints2, _ = cv2.projectPoints(objpoints[i], rvecs[i], tvecs[i],
                                               self.camera.color_intrinsics_mat, self.distCoeffs)
            error = cv2.norm(imgpoints[i], imgpoints2, cv2.NORM_L2) / len(imgpoints2)  # Normalize error
            total_error += error
        
        mean_error = total_error / len(objpoints)  # Average error across all points
        logger.info(f"重投影均方根误差: {mean_error}")
...
if __name__ == "__main__":  
    # 用户输入保存路径和内参
    base_save_dir = os.path.join(os.getcwd(), "calibration_results_left")  # 修改这个保存路径
    calib_L = 5   # 棋盘格内角点数量
    calib_W = 6  
    ...
    arm = Arm(ip="192.168.5.100", port=29999)  # 请修改为你的机械臂IP  
    try:  

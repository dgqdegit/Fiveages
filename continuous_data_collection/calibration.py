...
class Calibrator():  
    ...
    def calib(self, verbose: bool = True, save: bool = False, e2h: bool = True) -> np.ndarray:  
        ...
        if save:  
            np.save(os.path.join(self.external_matrix_save_dir, "calibrate_result.npy"), T)  
            np.savetxt(os.path.join(self.external_matrix_save_dir, "calibrate_result.txt"), T)  
        return T  

if __name__ == "__main__":  
    # 用户输入保存路径和内参
    base_save_dir = os.path.join(os.getcwd(), "calibration_results")  # 修改这个保存路径
    calib_L = 5   # 棋盘格内角点数量
    calib_W = 6  
